...
#include "bitplanes/core/internal/bitplanes_channel_subsampled.h"
...
BitPlanesChannelDataSubSampled<M>::set(const cv::Mat& src, const cv::Rect& roi,
                                       float s, float c1, float c2)
{
  ...
  int n_valid = GetNumValid(roi, _sub_sampling);
...
template <class M>
void BitPlanesChannelDataSubSampled<M>::computeResiduals(const cv::Mat& Iw,
...
getNormalization(const cv::Rect& roi, Transform& T, Transform& T_inv)
{
  Vector2f c(0,0);
